...
def get_next_video_id(playlistitems_list, vID, pID):
    ...
    for playlist_item in playlistitems_list["items"]:
        ...
        if (playlist_item["snippet"]["resourceId"]["videoId"] == vID and next is False) and \
                        playlistitems_list["pageInfo"]["resultsPerPage"] <= playlist_item["snippet"]["position"]:
            next = True
...
dataform = form.Form(
    form.Textbox("pID",
                 form.regexp("^PL",
                             "Must be a playlist ID, all of which start with 'PL'. Find it in the web address of the playlist page"),
