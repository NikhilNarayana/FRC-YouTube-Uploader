...
def create_description(options, blue1, blue2, blue3, blueScore, red1, red2, red3, redScore):
    ...
    if all(x <= -1 for x in (red1, red2, red3, redScore, blue1, blue2, blue3, blueScore)):
        return options.description % (
        options.ename, options.prodteam, options.twit, options.fb, options.weblink) + credits
    try:
...
def init(args):  # intializng all the variables where necessary and parsing data to create proper namespace fields
    ...
    if os.path.isfile(args.file):  # Check to make sure the file exists before continuing
        ...
        raise ValueError("The file doesn't exist, please check the name schema for more info")
    ...
    while response is None:
        try:
            ...
            if 'id' in response:
                ...
                rowRange = "Data!A1:F1"
                if type(options.end) is int:
                    wasBatch = "True"
                else:
                    wasBatch = "False"
                values = [
                    [str(datetime.now()), str(totalTime)[1:], "https://www.youtube.com/watch?v=%s" % response['id'],
                     str(options.tba), options.ename, wasBatch]]
                body = {'values': values}
                appendSpreadsheet = spreadsheet.spreadsheets().values().append(spreadsheetId=spreadsheetID,
                                                                               range=rowRange, valueInputOption="RAW",
                                                                               body=body).execute()
            else:
